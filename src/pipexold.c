...
/*   Updated: 2025/03/12 17:09:34 by jaacosta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "pipex.h"
...
void	setup_io(int i, int ac, char **av, int pipe_fd[2])
{
	int	infile = 0;
	int	outfile = 0;
...
static char	*find_command_path(char *command)
{
	...
	dir = strtok(path_copy, PATH_SEPARATOR);
	struct stat sb;
	if (!path)
		return (NULL);
	while (dir)
...
{
	char	*args[2];// = {av[i + 2], NULL}; // argumentos para execve
	char	*cmd_path;

	cmd_path = find_command_path(av[i + 2]);
	if (!cmd_path)
	{
		ft_printf("Error: comando no encontrado %s\n", av[i + 2]);
		exit(1);
	}
	args[0] = av[i + 2];
	args[1] = NULL;
...
void	wait_for_children(int process_count)
{
	int	i;
...
void	create_processes(int ac, char **av, char **envp)
{
	...
	{
		if (i != 0)
			pipe_fd[1] = create_pipe(pipe_fd);//crear un pipe por cada comando
		pid = fork();
		if (pid == -1)
		{
			perror("Error en fork");
		...
		if (pid == 0)
		{
			setup_io(i, ac, av, pipe_fd);
			execute_child_command(i, av, envp);
		}
		close(pipe_fd[1]);
		i++;
	}
	wait_for_children(ac - 3);
}
